...
#if 1
#if defined(__ARMCC_VERSION)                   /* MDK AC5/AC6 编译器 */
...
/* 不使用半主机模式，至少需要重定义_ttywrch\_sys_exit\_sys_command_string函数 */
int _ttywrch(int ch)
{
    ch = ch;
    return ch;
...
#else                                           /* GCC (arm-none-eabi-gcc) 编译器 */
/* GCC 下通过重写 _write 系统调用把 printf 重定向到串口 */
int _write(int fd, char *ptr, int len)
{
    ...
    for (i = 0; i < len; i++)
    {
        uint32_t t = 100000;                    /* 发送超时保护, 防止串口异常时卡死 */
        while ((USART1->SR & 0X40) == 0)
        {
            if (--t == 0) break;                /* 超时跳出, 避免死循环 */
        }
...
/* MDK 下重定义 fputc 函数, printf函数最终会通过调用fputc输出字符串到串口 */
