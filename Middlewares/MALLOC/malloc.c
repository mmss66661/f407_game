...
/* 内存池(64字节对齐)
 * GCC 移植说明:
 *   mem1base : 位于内部SRAM (普通 .bss)
 *   mem2base : 位于CCM RAM (0x10000000, 仅CPU可访问), 通过链接脚本 .ccmram section 定位
 *   mem3base : 位于外部SRAM (0x68000000, FSMC Bank3), 通过链接脚本 .extsram section 定位
 */
static uint8_t mem1base[MEM1_MAX_SIZE] __attribute__((aligned(64)));                 /* 内部SRAM内存池 */
static uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((aligned(64), section(".ccmram")));  /* 内部CCM内存池 */
static uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((aligned(64), section(".extsram"))); /* 外部SRAM内存池 */

/* 内存管理表 */
static MT_TYPE mem1mapbase[MEM1_ALLOC_TABLE_SIZE];                                   /* 内部SRAM内存池MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".ccmram")));    /* 内部CCM内存池MAP */
static MT_TYPE mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((section(".extsram")));   /* 外部SRAM内存池MAP */
